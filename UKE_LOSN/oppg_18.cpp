//   Fil:  FRODEH \ ALG_MET \ LAB_LOSN \ OPPG_18.CPP

//   L›sningsforslag til oppgave nr.18  i  Algoritmiske metoder.


//  Programmet er en utvidelse av 'EKS_12.CPP' slik at:
//     - n†r kortest vei mellom to noder er funnet
//     - s† traverseres grafen enda en gang og
//     - n†r man finner den f›rste stien som har denne lengden s†
//     - trekker man seg (baklengs) ut av rekursjonen igjen og
//     - key'en til nodene p† veien tilbake skrives ut.
...
void initAdjacent()  {      //  Initierer 'a' med alle nodenes naboer.
  ...
  a[25][24]=a[25][20]=a[25][19]=1;
			     ...
			     //  Aktuell node er 'fra', og man er p† leting etter 'til'-noden. 
                 //  N†v‘rende steg er nr.'steg', og minimum antall kanter funnet
			     //  til n† er 'minS'.
...
bool ferdig = false;              //  NYTT. For † fortelle n†r funnet korteste vei.

			     //  NYTT: Hele funksjonen er NY.
			     //  Funksjonen leter etter en sti som er 'minS' lang,
                 //  og som terminerer rekursjonen n†r denne er funnet.
void visit2(int fra, int til, int steg, int minS)  {
  int t;
  val[fra] = SEEN;           //  N† er 'fra' bes›kt (for † hindre cykler).
			                 //  NYTT:  IKKE 'minS-1' men bare 'minS':
  if (fra != til  &&  steg < minS)  {      //  Ikke n†dd fram, og fortsatt
					                       //    mulighet for † finne den
					                       //    korteste veien.
     for (t = 1;  t < MAXNODER;  t++)  {   //  Bes›ker (om mulig) naboene:
         if (a[fra][t] != 0  &&  val[t] == UNSEEN)
	         visit2(t, til, steg+1, minS);
	     if (ferdig) break;                //  NYTT. (N›ster ut av rekursjon.)
     }
					                       //  NYTT:
  } else if (fra == til && steg == minS )  ferdig = true;  // Korteste funnet.

  if (ferdig)  cout << fra << "  ";        //  NYTT. Skriver nodekey p† veien
					                       //        ut av rekursjonen.
  val[fra] = UNSEEN;         // 'Fra' er ferdig behandlet (i denne omgang)
}                            //    og andre kan bes›ke den senere (via
	             		     //    andre (og kanskje kortere) kanter inn).
...
int main()  {
  ...
  for (int i = 1;  i <= MAXNODER;  i++)  val[i] = UNSEEN;      //  NYTT
  cout << "\n\nKorteste vei g†r (bl.a.) gjennom nodene:  ";    //  NYTT
  visit2(fraNode, tilNode, 0, minSteg);                        //  NYTT
  cout << "\n\n\n\n";
  return 0;
}
