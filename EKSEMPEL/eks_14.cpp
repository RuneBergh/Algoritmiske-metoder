...
//  Fil:  FRODEH \ ALG_MET \ EKSEMPEL \ EKS_14.CPP

//  Eksemplet viser traversering av en vektet graf, for † finne et minimums
//  spenntre. Poenget er funksjonen kalt "visit" (se ALLER siste funksjon).
   ...
   while (!pq.empty())  {    //  Fortsatt ubehandlede (p† "fringen"):
       ...
       for (t = adj[k];  t != z;  t = t->next)   //  G†r gjennom alle naboer:
	       if (val[t->v] < 0)                    //  Naboen er p† "fringen"
						                         //    eller UNSEEN:
	          if (pq.update(t->v, t->w))  {      //  Ny lavere prioritet:
		         val[t->v] = -(t->w);            //    Oppdaterer med ny pri.
